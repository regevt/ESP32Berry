...
def patch_esp32berry_cpp(
    folder: str, class_suffix: str, title: str, index: int
) -> None:
    class_name = f"App{class_suffix}"
    new_case = (
        f"    case {index}:\n"
    ...
    if not m:
        print(
            "[ERROR] Could not find switch(menuNum) in ESP32Berry.cpp", file=sys.stderr
        )
        sys.exit(1)
...
"""
    ...
    if not m_for:
        print(
            "[ERROR] Could not find app buttons for-loop in UI main screen",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Scaffold and register a new ESP32Berry app."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Button index on main screen (0-4). Defaults to first free.",
    )
    parser.add_argument(
        "--title", type=str, default=None, help="Display title (defaults to app name)"
    )

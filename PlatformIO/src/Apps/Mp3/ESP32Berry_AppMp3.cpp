...
AppMp3::AppMp3(System *system, lv_obj_t *screen, const char *title)
...
void AppMp3::build_controls()
{
  ...
  lv_obj_set_width(lbl_track, Globals::get().screen_width - 20);
  ...
  lv_obj_set_width(slider_seek, Globals::get().screen_width - 140);
...
void AppMp3::update_progress_ui()
{
  ...
  if (lbl_time_left)
    lv_label_set_text(lbl_time_left, l);
  if (lbl_time_right)
    lv_label_set_text(lbl_time_right, r);
  ...
  if (btn_play)
  {
    lv_obj_t *child = lv_obj_get_child(btn_play, 0);
    if (child)
      lv_label_set_text(child, running ? "Pause" : "Play");
  }
}
...
void AppMp3::on_play_pause()
{
  ...
  if (btn_play)
  {
    lv_obj_t *child = lv_obj_get_child(btn_play, 0);
    if (child)
      lv_label_set_text(child, now ? "Pause" : "Play");
  }
}
